...
SECTIONS
{
    ...
    .rodata             : {} palign(8)      > DDR_MCU3_1
    .mpu_cfg                                > DDR_MCU3_1
    .cinit              : {} palign(8)      > DDR_MCU3_1
    .pinit              : {} palign(8)      > R5F_TCMA
    .bss                : {} align(4)       > DDR_MCU3_1
    .far                : {} align(4)       > DDR_MCU3_1
    ...
    .irqStack   : {. = . + __IRQ_STACK_SIZE;} align(4)      > DDR_MCU3_1  (HIGH)
    RUN_START(__IRQ_STACK_START)
    RUN_END(__IRQ_STACK_END)

    .fiqStack   : {. = . + __FIQ_STACK_SIZE;} align(4)      > DDR_MCU3_1  (HIGH)
    RUN_START(__FIQ_STACK_START)
    RUN_END(__FIQ_STACK_END)

    .abortStack : {. = . + __ABORT_STACK_SIZE;} align(4)    > DDR_MCU3_1  (HIGH)
    RUN_START(__ABORT_STACK_START)
    RUN_END(__ABORT_STACK_END)

    .undStack   : {. = . + __UND_STACK_SIZE;} align(4)      > DDR_MCU3_1  (HIGH)
    RUN_START(__UND_STACK_START)
    RUN_END(__UND_STACK_END)

    .svcStack   : {. = . + __SVC_STACK_SIZE;} align(4)      > DDR_MCU3_1  (HIGH)
